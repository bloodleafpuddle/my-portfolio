document.querySelectorAll('.project-card').forEach(card=>{const show=card.querySelector('.show');const hide=card.querySelector('.hide');show.addEventListener('click',e=>{e.preventDefault();card.classList.add('flipped')});hide.addEventListener('click',e=>{e.preventDefault();card.classList.remove('flipped')})});function validateForm(){let name=document.forms.myForm.name.value.trim();let email=document.forms.myForm.email.value.trim();let message=document.forms.myForm.message.value.trim();if(name===""){alert("Please enter your name!");return!1}
let atpos=email.indexOf("@");let dotpos=email.lastIndexOf(".");if(atpos<1||dotpos<atpos+2||dotpos+2>=email.length){alert("Please enter a valid email address!");return!1}
if(message.length<10){alert("Your message must be at least 10 characters long :)");return!1}
alert("Message sent successfully!");return!0}
function displayLocalDateTime(){const now=new Date();const formatted=now.toLocaleString(undefined,{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});const infoEl=document.getElementById("userInfo");const existingLocation=infoEl.dataset.location||"";infoEl.innerHTML=`Your current date and time: ${formatted}${existingLocation}`}
